<?php
	...
	if (!defined('TS_VCSC_NOTIFIER_PLUGIN_NAME')){
		define('TS_VCSC_NOTIFIER_PLUGIN_NAME', 					COMPOSIUM_NAME);
	}
	...
	if (!defined('TS_VCSC_NOTIFIER_PLUGIN_XML_FILE')){
		define('TS_VCSC_NOTIFIER_PLUGIN_XML_FILE', 				'http://www.maintenance.tekanewascripts.info/notifications/ts-notify-vc-extensions-wp.xml');
	}
		...
		function TS_VCSC_XML_UpdateNotifierMenuItem() {
			if (function_exists('simplexml_load_string')) {
				...
				if ((string) $xml->latest > (string) $plugin_data['Version']) {
					$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_ShowNotificationPage = "true";
				} else {
					$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_ShowNotificationPage = "false";
				}
			}	
		}
	}
	add_action('admin_menu', 'TS_VCSC_XML_UpdateNotifierMenuItem', 1);  
	...
	if (!function_exists('TS_VCSC_XML_UpdateNotifierAdminBar')){
		function TS_VCSC_XML_UpdateNotifierAdminBar() {
			if (function_exists('simplexml_load_string')) {
				...
				if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_UseUpdateMenuBarNotice == "true") {
					...
					if ((string)$xml->latest > (string) $plugin_data['Version']) {
						$wp_admin_bar->add_menu(array(
							...
							'href'								=> $VISUAL_COMPOSER_EXTENSIONS->settingsLink = "admin.php?page=TS_VCSC_Notification"
						));
					}
				}
			}
				...
				echo '<div class="ts-vcsc-notification-check-wrap">';
					echo '<div class="ts-vcsc-section-main">';
						...
						echo '<div class="ts-vcsc-section-content">';
							...
							forum, provided you created an account and registered your license key for authorization (one time only; requires active support package for final access). All updates will also (eventually) be delivered via auto-update, provided you entered and confirmed your
							...
							echo '<div style="text-align: justify; margin-bottom: 20px;">';
								if ((string) $xml->latest <= (string) $xml->forum) {
									...
									echo '<div>To download the updated version, login to our <a href="http://helpdesk.tekanewascripts.com/" target="_blank"">Support Forum</a>, head over to the <a href="http://helpdesk.tekanewascripts.com/freebies-page/" target="_blank">Downloads</a> page and download the plugin update from the provided listing of plugin releases.
									<strong>Please note that accessing the forum and downloads page requires a valid support package from Envato.</strong></div>';
								} else {
							...
							echo '<div style="text-align: justify; margin-bottom: 20px;">';
								if ((string) $xml->latest <= (string) $xml->codecanyon) {
									...
									echo '<div>To download the updated version, log into <a href="//codecanyon.net/?ref=Tekanewa" target="_blank"">CodeCanyon</a>, head over to your <a href="http://codecanyon.net/downloads" target="_blank">Downloads</a> section and re-download the plugin like you did when you first bought it.</div>';
								} else {
