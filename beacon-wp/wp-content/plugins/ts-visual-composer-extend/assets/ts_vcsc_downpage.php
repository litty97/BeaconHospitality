<?php
	...
	if (!class_exists('Mobile_Detect')) {
		require_once($VISUAL_COMPOSER_EXTENSIONS->detector_dir . 'ts_mobile_detect.php');
	}
	if (class_exists('Mobile_Detect')) {
		$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_MobileDetector_Global		= new Mobile_Detect;
		$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_MobileDetector_Mobile		= ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_MobileDetector_Global->isMobile() == 1 ? 'true' : 'false');
		$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_MobileDetector_Tablet		= ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_MobileDetector_Global->isTablet() == 1 ? 'true' : 'false');
		if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_MobileDetector_Mobile == 'false') {
			$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_MobileDetector_Desktop	= 'true';
		} else {
			$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_MobileDetector_Desktop	= 'false';
		}
		unset($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_MobileDetector_Global);
	}
	...
	<div class="wrapper ts-vcsc-settings-group-container">		
		<div class="ts-vcsc-settings-group-header">
			<div class="display_header">
				<h2><span class="dashicons dashicons-clock"></span>Composium - Visual Composer Extensions v<?php echo TS_VCSC_GetPluginVersion(); ?> ... Downtime Manager (BETA)</h2>
			</div>
			<div class="clear"></div>
		</div>
		<div class="ts-vcsc-settings-group-topbar ts-vcsc-settings-group-buttonbar">
			<a href="javascript:void(0);" id="ts-vcsc-settings-group-toggle" class="ts-vcsc-settings-group-toggle" data-toggle-status="false" data-downtime-status="<?php echo ($ts_vcsc_extend_settings_downpage_active == 1 ? 'enabled' : 'disabled'); ?>">Expand</a>
		...
		<div id="v-nav" class="ts-vcsc-settings-group-tabs">
			<ul id="v-nav-main" data-type="settings">
				<li id="link-ts-settings-logo" class="first" style="border-bottom: 1px solid #DDD; height: 230px;">
					<img style="width: 100%; height: auto; margin: 0 auto;" src="<?php echo TS_VCSC_GetResourceURL('images/other/downtime_icon_01.png'); ?>">
				</li>
