				...
				<?php
					if ($TS_VCSC_ComposerSectionElement == "true") {
						echo '<div class="ts-vcsc-notice-field ts-vcsc-success" style="margin-top: 10px; font-size: 13px; text-align: justify;">
							"Composium - Visual Composer Extensions" allows you to extend the available options for section, row and column settings, adding features such as viewport animations (section, row and column) and a variety of background effects (section + row). If you already use other plugins that provide the same or similar options you should decide for either one but not use both at the same time as they can cause contradicting settings. Also, if your theme incorporates Visual Composer by itself, some themes already provide you with similar options; in these cases, you should disable the settings below in order to avoid any conflicts.
						</div>';
						echo '<div style="margin-top: 20px; font-weight: bold;">The extended row and column options require a Visual Composer version of 5.0 or higher, in order to function correctly!</div>';
					} else {
						echo '<div class="ts-vcsc-notice-field ts-vcsc-success" style="margin-top: 10px; font-size: 13px; text-align: justify;">
							"Composium - Visual Composer Extensions" allows you to extend the available options for row and column settings, adding features such as viewport animations (row + column) and a variety of background effects (row). If you already use other plugins that provide the same or similar options you should decide for either one but not use both at the same time as they can cause contradicting settings. Also, if your theme incorporates Visual Composer by itself, some themes already provide you with similar options; in these cases, you should disable the settings below in order to avoid any conflicts.
						</div>';
						echo '<div style="margin-top: 20px; font-weight: bold;">The extended section, row and column options require a Visual Composer version of 4.3 or higher, in order to function correctly!</div>';
					}
				?>
        ...
        <div class="ts-vcsc-section-main">
			...
			<div class="ts-vcsc-section-content">
				<?php
					if ($TS_VCSC_ComposerSectionElement == "true") {
						echo '<div class="ts-vcsc-notice-field ts-vcsc-warning" style="margin-top: 10px; font-size: 13px; text-align: justify;">
							The amount of available additional options for section and row settings is very extensive and can slow down the rendering of the row settings panel in Visual Composer, particularly, if all available options are utilized. The controls below allow you to easily define your custom set of additional section and row options, which can significantly decrease the rendering time for the settings panel in Visual Composer, particularly, if only a few options ar added.
						</div>';
					} else {
						echo '<div class="ts-vcsc-notice-field ts-vcsc-warning" style="margin-top: 10px; font-size: 13px; text-align: justify;">
							The amount of available additional options for row settings is very extensive and can slow down the rendering of the row settings panel in Visual Composer, particularly, if all available options are utilized. The controls below allow you to easily define your custom set of additional row options, which can significantly decrease the rendering time for the settings panel in Visual Composer, particularly, if only a few options ar added.
						</div>';
					}
				?>	
				<div style="margin-top: 10px; margin-bottom: 10px;">
					<?php
						if ($TS_VCSC_ComposerSectionElement == "true") {
							echo '<div style="font-weight: bold; font-size: 14px; margin: 0;">Extend Options for Visual Composer Sections + Rows:</div>';
					...
					<div id="ts_vcsc_extend_settings_rowAllowableGlobalsWrap" class="ts-multiselect-holder" style="display: block; height: auto; margin: 10px 0 10px 0; border: 1px solid #ededed; padding: 10px 20px; background: #f9f9f9;">
						...
						<?php
							if ($TS_VCSC_ComposerNativeAnimation == "true") {
								echo '<div class="ts-vcsc-notice-field ts-vcsc-warning" style="margin-top: 10px; font-size: 13px; text-align: justify;">
									Visual Composer introduced section + row viewport animations with v5.0.0 as native feature. While the section + row viewport animation feature from this addon does allow for some additional controls over the viewport animation that Visual Composer does not, it is always better to use a native feature if available. If you are using the section + row viewport animation from this addon, ensure that you don not also use the native viewport animation option from Visual Composer for the same section or row at the same time in order to avoid conflicts.
								</div>';
							}
						?>						
						...
						<div id="ts_vcsc_extend_settings_rowAllowableVisibilityWrap" style="display: block; width: 100%; margin: 20px 0 0 0;">
							<?php
								if ($TS_VCSC_ComposerSectionElement == "true") {
									...
									echo '<div class="ts-vcsc-notice-field ts-vcsc-warning" style="margin-top: 10px; font-size: 13px; text-align: justify;">
										As the section and row shortcode is actually defined and handled by Visual Composer itself and due to the way Visual Composer allows add-ons to extend section and row options, it is NOT possible to apply the row visibility check server side, but only via JS function (client side).
									</div>';
								} else {
									...
									echo '<div class="ts-vcsc-notice-field ts-vcsc-warning" style="margin-top: 10px; font-size: 13px; text-align: justify;">
										As the row shortcode is actually defined and handled by Visual Composer itself and due to the way Visual Composer allows add-ons to extend row options, it is NOT possible to apply the row visibilit check server side, but only via JS function (client side).
									</div>';
								}
							?>					
			...
			<div class="ts-vcsc-section-content">
				<div style="margin-top: 10px; margin-bottom: 20px;">
					...
					<?php
						if ($TS_VCSC_ComposerNativeAnimation == "true") {
							echo '<div class="ts-vcsc-notice-field ts-vcsc-warning" style="margin-top: 10px; font-size: 13px; text-align: justify;">
								Visual Composer introduced column viewport animations with v5.0.0 as native feature. While the column viewport animation feature from this addon does allow for some additional controls over the viewport animation that Visual Composer does not, it is always better to use a native feature if available. If you are using the column viewport animation from this addon, ensure that you don not also use the native viewport animation option from Visual Composer for the same column at the same time in order to avoid conflicts.
							</div>';
						}
					?>
