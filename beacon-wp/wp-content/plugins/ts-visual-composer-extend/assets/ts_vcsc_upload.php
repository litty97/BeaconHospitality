				...
				if (($basicCheck == true) && ($filesFound == true)) {
					...
					if (!is_wp_error($Custom_JSON) && !empty($Custom_JSON)) {
						...
						if ((isset($Custom_Code['IcoMoonType'])) && (isset($Custom_Code['icons'])) && ($Custom_MultiColored == "false")){
							...
							echo '<script>
								jQuery(document).ready(function() {
									jQuery(".fontPackUploadedSuccess").parent("div").after("<div class=error><p>This font includes dual- or multi-colored icons, which are not supported by Visual Composer. Please use only standard (single) colored icons.</p></div>");
								});
							</script>';
							TS_VCSC_CustomFontImportMessages('warning', 'This font includes dual- or multi-colored icons, which are not supported by Visual Composer. Please use only standard (single) colored icons. ' . $upload_debugtxt . '');
						} else {
							TS_VCSC_ResetCustomFont();
							echo '<script>
								jQuery(document).ready(function() {
...
?>
	...
	<div class="ts-vcsc-section-main">
		...
		<div class="ts-vcsc-section-content">
			...
			<div id="ts_vcsc_icons_upload_system_trigger" class="clearFixMe" style="margin-top: 20px;">
