<?php
	...
	function TS_VCSC_Lightbox_Image_Function ($atts, $content = null) {
		...
		if (function_exists('vc_shortcode_custom_css_class')) {
			if ($content_shape == "standard") {
				$css_class 					= apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'nchgrid-item nchgrid-tile nch-lightbox-single nch-lightbox-image ' . $modal_id . '-parent ' . $el_class . ' ' . vc_shortcode_custom_css_class($css, ' '), 'TS-VCSC-Lightbox-Image', $atts);
			} else {
			...
			if ($content_shape == "standard") {
				$css_class					= 'nchgrid-item nchgrid-tile nch-lightbox-single nch-lightbox-image ' . $modal_id . '-parent ' . $el_class;
			} else {
				$css_class					= 'ts-lightbox-shape-container ' . $modal_id . '-parent ' . $el_class;
			}
		}
		
		$output .= '<div id="' . $modal_id . '" class="' . $css_class . ' ' . $overlay_visible . ' nch-lightbox-hover-' . $overlay_animation . '" style="margin-top: ' . $margin_top . 'px; margin-bottom: ' . $margin_bottom . 'px; ' . $parent_dimensions . ' ' . $parent_alignment . '">';
			$output .= '<a href="' . $modal_image . '" class="nch-lightbox-media nofancybox no-ajaxy" data-title="' . $content_lightbox . '" rel="' . ($lightbox_group == "true" ? "nachogroup" : $lightbox_group_name) . '" data-thumbnail="' . $modal_thumb . '" data-usecors="' . $lightbox_usecors . '" data-share="' . ($lightbox_social == "true" ? 1 : 0) . '" data-effect="' . $lightbox_effect . '" data-duration="' . $lightbox_speed . '" ' . $nacho_color . '>';
				if ($content_shape == "standard") {
					$output .= '<img class="nchgrid-image-' . $overlay_animation . '" src="' . $modal_thumb . '" ' . $modal_retina . ' alt="' . $alt_attribute . '" title="" style="display: block; ' . $image_dimensions . '">';
					$output .= '<div class="nchgrid-caption ' . $overlay_classes . '" style="' . $overlay_background . ' ' . $overlay_styling . '">' . $overlay_addition . '</div>';
					if (!empty($content_title)) {
						$output .= '<div class="nchgrid-caption-text" style="background: ' . $overlay_title_back . '; color: ' . $overlay_title_color . ';">' . $content_title . '</div>';
					}
				} else {
