<?php
	...
	function TS_VCSC_Font_Iconbox_Tiny_Function ($atts, $content = null) {
		...
		extract(shortcode_atts( array(
			...
			'box_border_thick'          => 1,
			'box_border_radius'         => '',
			'box_border_setting'		=> 'border-top-style:solid;border-right-style:solid;border-bottom-style:solid;border-left-style:solid;|border-top-width:1px;border-top-left-radius:0px;border-right-width:1px;border-top-right-radius:0px;border-bottom-width:1px;border-bottom-right-radius:0px;border-left-width:1px;border-bottom-left-radius:0px;|border-top-color:#000000;border-right-color:#000000;border-bottom-color:#000000;border-left-color:#000000;',
		...
		$icon_frame_top = $icon_frame_side = $box_margin_left = $box_padding_side = "0px";
		...
		$icon_padding					= $icon_padding_horizontal + $icon_padding_horizontal + $icon_size_slide + $icon_margin;
		...
		if (($style == "icon_left") && ($content_align == "true") && ($title_align == "left")) {
			$content_align				= "padding-left: " . $icon_padding . "px;";	
		} else if (($style == "icon_right") && ($content_align == "true") && ($title_align == "right")) {
			$content_align				= "padding-right: " . $icon_padding . "px;";	
		} else {
			$content_align				= "";
		}
		...
		if ($style == "boxed") {
			if ($icon_location == "top") {
				...
				$icon_frame_side        = '-' . ($icon_size_slide / 2 + $icon_padding_vertical + $icon_frame_thick) . 'px';
				$icon_frame_adjust      = 'top:' . $icon_frame_top . '; margin-left:' . $icon_frame_side . ';';
				$box_frame_adjust       = '';
				$shadow_frame_adjust    = '';
				...
				$icon_frame_side        = '-' . ($icon_size_slide / 2 + $icon_padding_horizontal + $icon_frame_thick + 5) . 'px';
				if ($icon_placement == 'center') {
					$icon_frame_adjust	= 'left:' . $icon_frame_side . '; margin-top:' . $icon_frame_top . ';';
				} else if ($icon_placement == 'top') {
					$icon_frame_adjust	= 'left:' . $icon_frame_side . '; top: 0px; bottom: auto;';
				} else if ($icon_placement == 'bottom') {
					$icon_frame_adjust	= 'left:' . $icon_frame_side . '; top: auto; bottom: 0px;';
				}
				$box_margin_left        = ($icon_size_slide / 2) . 'px';
				$box_padding_top		= '';
				if ($padding_custom == "true") {
					$box_padding_side	= $padding_outside . 'px';
				} else {
					$box_padding_side	= (($icon_size_slide / 2) + 30) . 'px';
				}				
				if ($box_border_type == '') {
					$box_frame_adjust	= 'margin-left: 0px; padding: 0 0 0 ' . $box_padding_side . ';';
				} else {
					$box_frame_adjust	= 'margin-left: 0px; padding-left: ' . $box_padding_side . ';';
				}
				...
				$icon_frame_side        = '-' . ($icon_size_slide / 2 + $icon_padding_horizontal + $icon_frame_thick + 5) . 'px';
				if ($icon_placement == 'center') {
					$icon_frame_adjust	= 'right:' . $icon_frame_side . '; margin-top:' . $icon_frame_top . ';';
				} else if ($icon_placement == 'top') {
					$icon_frame_adjust	= 'right:' . $icon_frame_side . '; top: 0px; bottom: auto;';
				} else if ($icon_placement == 'bottom') {
					$icon_frame_adjust	= 'right:' . $icon_frame_side . '; top: auto; bottom: 0px;';
				}
				$box_margin_left        = ($icon_size_slide / 2) . 'px';
				$box_padding_top		= '';
				if ($padding_custom == "true") {
				...
				if ($box_border_type == '') {
					$box_frame_adjust	= 'margin-right: 0px; padding: 0 0 0 ' . $box_padding_side . ';';
				} else {
					$box_frame_adjust	= 'margin-right: 0px; padding-right: ' . $box_padding_side . ';';
				}
				$shadow_frame_adjust    = 'margin-right: ' . $box_margin_left . '; ';
				if ($separator_type != '') {
					$separator_style	= 'border-right: ' . $separator_thick . 'px ' . $separator_type . ' ' . $separator_color . '; padding-right: ' . abs(($icon_size_slide / 2 + $icon_padding_horizontal + $icon_frame_thick + 5)) . 'px;';
				}
			}
		}
		...
		} else if ($box_background_type == "gradient") {
			$box_background_style		= $box_background_gradient;
					...
					if ($style == "icon_left") {
						$output .= '<div class="ts-css-shadow ' . $animation_shadow . ' ' . $box_border_radius . '" style="' . $box_background_style . '">';
							$output .= '<div class="ts-icon-box-inner box-detail-wrapper ' . $box_border_radius . ' ' . $height_class . '" style="' . $box_frame_style . ' ' . $box_inner_padding . ' ' . $height_setting . '">';
								$output .= '<div class="ts-box-icon-title-holder" style="display: table; margin: 0 auto ' . $title_spacing . 'px auto; float: ' . ($title_align == 'center' ? 'none' : $title_align) . ';">';
								...
								$output .= '</div>';
								$output .= '<div class="ts-icon-box-content" style="clear: both; ' . $content_align . ' font-size: ' . $font_size . 'px; line-height: ' . $line_height . 'px; ' . $google_font_content . '">';
							...
							$output .= '<div class="ts-icon-box-inner top-side ' . $animation_css . ' ' . $box_border_radius . ' ' . $height_class . '" style="' . $box_frame_style . ' ' . $box_inner_padding . ' ' . $height_setting . '">';
								...
								$output .= '<div class="box-detail-wrapper" style="clear: both;">';
									if (($read_more_url != '') && (($read_more_link == "buttontitle") || ($read_more_link == "title"))) {
										...
										$output .= '<' . $title_wrap . ' class="ts-box-icon-title" style="width: auto !important; color:' . $title_color . '; font-size:' . $title_size . 'px; font-weight:' . $title_weight . '; margin-bottom: ' . $title_spacing . 'px; text-align:' . $title_align . '; ' . $google_font_title . '">' . $title . '</' . $title_wrap . '>';
									if (($read_more_url != '') && (($read_more_link == "buttontitle") || ($read_more_link == "title"))) {
										$output .= '</a>';
									}	
								if (function_exists('wpb_js_remove_wpautop')){
									$output .= '<div class="ts-icon-box-content" style="margin-top: 10px; font-size: ' . $font_size . 'px; line-height: ' . $line_height . 'px; ' . $google_font_content . '">' . wpb_js_remove_wpautop(do_shortcode($content), $wpautop) . '</div>';
								} else {
								...
								if ($separator_type != '') {
									...
									$output .= '<div class="ts-icon-box-inner ' . $height_class . '" style="' . $height_setting . '">';
										if (($read_more_url != '') && (($read_more_link == "buttontitle") || ($read_more_link == "title"))) {
											...
											$output .= '<' . $title_wrap . ' class="ts-box-icon-title" style="color:' . $title_color . '; font-size:' . $title_size . 'px; font-weight:' . $title_weight . '; margin-top:' . $title_margin . 'px; margin-bottom: ' . $title_spacing . 'px; text-align:' . $title_align . '; ' . $google_font_title . '">' . $title . '</' . $title_wrap . '>';
										if (($read_more_url != '') && (($read_more_link == "buttontitle") || ($read_more_link == "title"))) {
											$output .= '</a>';
										}	
										if (function_exists('wpb_js_remove_wpautop')){
