	...
	if (!class_exists('TS_Testimonials')){
		class TS_Testimonials {
			...
			function TS_VCSC_Testimonial_Frontend_Form($atts, $content = null) {
				...
				if ((isset($_POST['submit'])) && ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode == "false")) {
				...
				$output .= '<div id="' . $testimonial_form_id . '" class="' . $css_class . '" style="margin-top: ' . $margin_top . 'px; margin-bottom: ' . $margin_bottom . ';">';
					$output .= '<form id="ts-testimonial-frontend-submission-form-' . $testimonial_random . '" class="ts-testimonial-frontend-submission-form" name="ts-testimonial-frontend-submission-form" autocomplete="' . $form_complete . '" enctype="multipart/form-data" method="POST" action="">';
						...
						// Submit Button
						if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode == "true") {
							$output .= '<input class="' . $button_style . ' ' . $button_hover . '" data-frontend="' . $VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode . '" type="button" value="' . $button_submit . '" tabindex="6" id="submit" name="submit" />';
						} else {
							$output .= '<input class="' . $button_style . ' ' . $button_hover . '" data-frontend="' . $VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode . '" type="submit" value="' . $button_submit . '" tabindex="6" id="submit" name="submit" />';
						}
						// NONCE + Action
			   ...
			   exit;
				...
				if (($form_active == "true") && ($error_pass == "true")) {
					...
					update_post_meta($testimonial, 'ts_vcsc_testimonial_basic', 			array(
						'ts_vcsc_testimonial_basic_author' 			=> $author,
						'ts_vcsc_testimonial_basic_position' 		=> $_POST['position']
					));
