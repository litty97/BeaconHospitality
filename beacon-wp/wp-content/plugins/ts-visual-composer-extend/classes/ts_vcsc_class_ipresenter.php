<?php
	if (!class_exists('TS_iPresenter')){
		class TS_iPresenter {
			function __construct() {
				...
				if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode == "true") {
					if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_LeanMap == "true") {
						$this->TS_VCSC_Add_iPresenter_Lean();
					} else if (function_exists('vc_map')) {
						add_action('init',									array($this, 'TS_VCSC_Add_iPresenter_Element_Container'), 9999999);
						add_action('init',									array($this, 'TS_VCSC_Add_iPresenter_Element_Item'), 9999999);
					}
				} else {
					if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_LeanMap == "true") {
						add_action('admin_init',							array($this, 'TS_VCSC_Add_iPresenter_Lean'), 9999999);
					} else if (function_exists('vc_map')) {
						add_action('admin_init',							array($this, 'TS_VCSC_Add_iPresenter_Element_Container'), 9999999);
						add_action('admin_init',							array($this, 'TS_VCSC_Add_iPresenter_Element_Item'), 9999999);
					}
				}
			...
			function TS_VCSC_iPresenter_Container ($atts, $content = null){
				...
				if ($frontend_edit == "false") {
					...
					$output .= '<div id="' . $ipresenter_id . '" class="' . $css_class . ' clearFixMe" style="margin-top: ' . $margin_top . 'px; margin-bottom: ' . $margin_bottom . 'px;">';
						$output .= '<div id="ts-ipresenter-container-' . $ipresenter_random . '" class="ts-ipresenter-container clearFixMe" style="" ' . $general_data . ' ' . $navigation_data . ' ' . $autoplay_data . ' ' . $timer_data . '>';
							$output .= '<div id="ts-ipresenter-main-' . $ipresenter_random . '" class="ts-ipresenter-main ' . ($animation_allow == 'true' ? "ts-ipresenter-main-animated" : "ts-ipresenter-main-fade") . ' clearFixMe">';
								if (function_exists('wpb_js_remove_wpautop')){
									$output .= wpb_js_remove_wpautop(do_shortcode($content), $wpautop);
								} else {
									$output .= do_shortcode($content);
					...
					$output .= '<div id="' . $ipresenter_id . '" class="' . $css_class . ' clearFixMe" style="margin-top: ' . $margin_top . 'px; margin-bottom: ' . $margin_bottom . 'px;">';
						$output .= '<div id="ts-ipresenter-container-' . $ipresenter_random . '" class="ts-ipresenter-container clearFixMe" style="">';
							$output .= '<div id="ts-ipresenter-main-frontend-' . $ipresenter_random . '" class="ts-ipresenter-main-frontend clearFixMe">';
								if (function_exists('wpb_js_remove_wpautop')){
									$output .= wpb_js_remove_wpautop(do_shortcode($content), $wpautop);
								} else {
									$output .= do_shortcode($content);
			...
			function TS_VCSC_Add_iPresenter_Element_Container() {
				global $VISUAL_COMPOSER_EXTENSIONS;
				...
				$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_Element = array(
					...
					"class"                             => "",
					"icon"                              => "ts-composer-element-icon-ipresenter-main",
