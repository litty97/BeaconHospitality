<?php
	if (!class_exists('TS_Skillsets')){
		class TS_Skillsets {
			function __construct() {
				...
				if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode == "true") {
					if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_LeanMap == "true") {
						$this->TS_VCSC_Add_Skill_Sets_Element_Lean();
					} else if (function_exists('vc_map')) {
						add_action('init',									array($this, 'TS_VCSC_Add_Skill_Sets_Element_Bars'), 9999999);
						add_action('init',									array($this, 'TS_VCSC_Add_Skill_Sets_Element_Raphael'), 9999999);
					}
				} else {
					if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_LeanMap == "true") {
						add_action('admin_init',							array($this, 'TS_VCSC_Add_Skill_Sets_Element_Lean'), 9999999);
					} else if (function_exists('vc_map')) {
						add_action('admin_init',							array($this, 'TS_VCSC_Add_Skill_Sets_Element_Bars'), 9999999);
						add_action('admin_init',							array($this, 'TS_VCSC_Add_Skill_Sets_Element_Raphael'), 9999999);
					}
				}
			...
			function TS_VCSC_Add_Skill_Sets_Element_Lean() {
				vc_lean_map('TS_VCSC_Skill_Sets_Standalone', 				array($this, 'TS_VCSC_Add_Skill_Sets_Element_Bars'), null);
				vc_lean_map('TS_VCSC_Skill_Sets_Raphael', 					array($this, 'TS_VCSC_Add_Skill_Sets_Element_Raphael'), null);
			}
			...
			function TS_VCSC_Skill_Sets_Standalone ($atts, $content = null) {
				...
				$codestarMigrated 					= get_post_meta($skillset_id, 'ts_vcsc_custompost_migrated', true);
				...
				if ($codestarRetrieve == "true") {
					$custom_fields 					= get_post_meta($Skill_ID, 'ts_vcsc_skillset_basic', true);
					$custom_fields_array			= array();
					if (isset($custom_fields['ts_vcsc_skillset_basic_group'])) {
						$Skill_Group				= $custom_fields['ts_vcsc_skillset_basic_group'];
					} else {
						$Skill_Group				= array();
					}
				} else {
				...
				$skill_background 					= '';
				if ($codestarRetrieve == "true") {
					$skill_entries					= $Skill_Group;
				} else if (isset($Skill_Group)) {
			...
			function TS_VCSC_Add_Skill_Sets_Element_Bars() {
				global $VISUAL_COMPOSER_EXTENSIONS;
				// Add Standalone Skillset
				$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_Element = array(
					"name"                              => __( "TS Skillset Bars", "ts_visual_composer_extend" ),
					"base"                              => "TS_VCSC_Skill_Sets_Standalone",
					"icon" 	                            => "ts-composer-element-icon-skillset-standalone",
					"class"                             => "",
			...
			function TS_VCSC_Add_Skill_Sets_Element_Raphael() {
				global $VISUAL_COMPOSER_EXTENSIONS;
				// Add Raphael Skillset
				$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_Element = array(
					"name"                              => __( "TS Skillset Raphael", "ts_visual_composer_extend" ),
					"base"                              => "TS_VCSC_Skill_Sets_Raphael",
					"icon" 	                            => "ts-composer-element-icon-skillset-raphael",
					"class"                             => "",
					"category"                          => __( 'VC Extensions', "ts_visual_composer_extend" ),
