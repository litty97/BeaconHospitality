<?php
	if (!class_exists('TS_Timelines')){
		class TS_Timelines {
			function __construct() {
				...
				if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode == "true") {
					if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_LeanMap == "true") {
						$this->TS_VCSC_Add_Timeline_Elements_Lean();
					} else if (function_exists('vc_map')) {
						add_action('init',									array($this, 'TS_VCSC_Add_Timeline_Element_Container'), 9999999);
						add_action('init',									array($this, 'TS_VCSC_Add_Timeline_Element_Single'), 9999999);
						add_action('init',									array($this, 'TS_VCSC_Add_Timeline_Element_Break'), 9999999);
					}
				} else {
					if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_LeanMap == "true") {
						add_action('admin_init',							array($this, 'TS_VCSC_Add_Timeline_Elements_Lean'), 9999999);
					} else if (function_exists('vc_map')) {
						add_action('admin_init',							array($this, 'TS_VCSC_Add_Timeline_Element_Container'), 9999999);
						add_action('admin_init',							array($this, 'TS_VCSC_Add_Timeline_Element_Single'), 9999999);
						add_action('admin_init',							array($this, 'TS_VCSC_Add_Timeline_Element_Break'), 9999999);
					}
				}
			...
			function TS_VCSC_Add_Timeline_Elements_Lean() {
				vc_lean_map('TS_VCSC_Timeline_Container', 					array($this, 'TS_VCSC_Add_Timeline_Element_Container'), null);
				vc_lean_map('TS_VCSC_Timeline_Single',						array($this, 'TS_VCSC_Add_Timeline_Element_Single'), null);
				vc_lean_map('TS_VCSC_Timeline_Break',						array($this, 'TS_VCSC_Add_Timeline_Element_Break'), null);
			}
											...
											if (!empty($content)) {
												$output .= '<div class="ts-timeline-text-wrap-inner" style="' . (empty($icon) ? "width: 100%; height: 100%; left: 0;" : " left: 0;") . '">';
													if (function_exists('wpb_js_remove_wpautop')){
														$output .= '<div class="ts-timeline-text" style="">' . wpb_js_remove_wpautop(do_shortcode($content), $wpautop) . '</div>';
													} else {
			...
			function TS_VCSC_Add_Timeline_Element_Container() {
				global $VISUAL_COMPOSER_EXTENSIONS;
			...
			function TS_VCSC_Add_Timeline_Element_Break() {
				global $VISUAL_COMPOSER_EXTENSIONS;
