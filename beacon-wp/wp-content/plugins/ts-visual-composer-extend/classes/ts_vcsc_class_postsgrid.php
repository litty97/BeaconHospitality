<?php
	if (!class_exists('TS_Postgrids')){
		class TS_Postgrids {
			function __construct() {
				...
				if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode == "true") {
					if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_LeanMap == "true") {
						$this->TS_VCSC_Add_Posts_Grid_Lean();
					} else if (function_exists('vc_map')) {
						add_action('init',                                  array($this, 'TS_VCSC_Add_Posts_Grid_Elements'), 9999999);
					}
				} else {
					if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_LeanMap == "true") {
						add_action('admin_init',							array($this, 'TS_VCSC_Add_Posts_Grid_Lean'), 9999999);
					} else if (function_exists('vc_map')) {
			...
			function TS_VCSC_Add_Posts_Grid_Lean() {
				vc_lean_map('TS_VCSC_Posts_Grid_Standalone',				array($this, 'TS_VCSC_Add_Posts_Grid_Elements'), null);
			}
				...
				if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode == 'true') {
					echo '<div id="ts-isotope-posts-grid-frontend-' . $postsgrid_random . '" class="ts-isotope-posts-grid-frontend" style="border: 1px solid #ededed; padding: 10px;">';
						...
						if ($limit_posts == 'true') {
							...
							echo '<div>' . __( "Include", "ts_visual_composer_extend" ) . ': ' . (empty($limit_include) ? __( 'None', "ts_visual_composer_extend" ) : $limit_include) . '</div>';
						}
			...
			function TS_VCSC_Add_Posts_Grid_Elements() {
				...
				$VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_Element = array(
					...
					"icon" 	                            => "ts-composer-element-icon-isotope-posts",
