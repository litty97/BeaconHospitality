<?php
	if (!class_exists('TS_Teammates_Old')){
		class TS_Teammates_Old {
			function __construct() {
				...
				if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VCFrontEditMode == "true") {
					if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_LeanMap == "true") {
						$this->TS_VCSC_Add_Teammate_Old_Lean();
					} else if (function_exists('vc_map')) {
						add_action('init',                                  array($this, 'TS_VCSC_Add_Teammate_Old_Elements'), 9999999);
					}
				} else {
					if ($VISUAL_COMPOSER_EXTENSIONS->TS_VCSC_VisualComposer_LeanMap == "true") {
						add_action('admin_init',							array($this, 'TS_VCSC_Add_Teammate_Old_Lean'), 9999999);
					} else if (function_exists('vc_map')) {
			...
			function TS_VCSC_Add_Teammate_Old_Lean() {
				vc_lean_map('TS-VCSC-Team-Mates', 							array($this, 'TS_VCSC_Add_Teammate_Old_Elements'), null);
			}
					...
					if ((isset($Team_Buttonfile)) || (isset($Team_Attachment))) {
						...
						if (!empty($Team_File)) {
							...
							if (isset($Team_Buttontooltip)) {
								$team_download 	.= '<a class="ts-teammate-file-link ts-button ' . $Team_Buttontype . ' ' . $team_tooltipclasses . '" data-tooltipster-text="' . $Team_Buttontooltip . '" ' . $team_tooltipcontent . ' href="' . $Team_File . '" target="_blank"><img class="ts-teammate-file-image" src="' . TS_VCSC_GetResourceURL('images/filetypes/' . $Team_FileFormat . '.png') . '"> ' . $Team_Buttonlabel . '</a>';
							} else {
