...
if (!defined('COMPOSIUM_VERSION')){
	define('COMPOSIUM_VERSION', 			'5.0.8');
}
...
if (!class_exists('VISUAL_COMPOSER_EXTENSIONS')) {
	...
	class VISUAL_COMPOSER_EXTENSIONS {
		...
		function __construct() {
			...
			//add_action('vc_after_init', 					'TS_VCSC_IconFontsIntegrateVC');
			//add_filter('vc_iconpicker-type-customuserfont','TS_VCSC_IconFontsAddFilter');
			...
			// Remove Auto Paragrpahs from Content + Excerpts
			// ----------------------------------------------
			...
			foreach ($this->TS_VCSC_Icon_Font_Settings as $Icon_Font => $iconfont) {
				if ($iconfont['setting'] != 'Custom') {
					...
					if ((!isset($this->{'TS_VCSC_Icons_' . $iconfont['setting'] . ''})) && (($this->{'TS_VCSC_tinymce' . $iconfont['setting'] . ''} == 1) || ($TS_VCSC_IconFont_Override == 1))) {
						if (($this->TS_VCSC_VisualComposer_Loading == "true") || ($this->TS_VCSC_Icons_Compliant_Loading == "true")) {
		...
		function TS_VCSC_Extensions_Create_Variables() {
			$TS_VCSC_Global_Variables_Output	= '';
			...
			echo '<script type="text/javascript">';
				...
				if ($this->TS_VCSC_UseInternalLightbox == "true") {
					...
					echo 'var $TS_VCSC_Lightbox_URLColor = ' . 					(((array_key_exists('urlcolorscan', $TS_VCSC_Lightbox_Defaults)) ? 			$TS_VCSC_Lightbox_Defaults['urlcolorscan'] : 			$this->TS_VCSC_Lightbox_Setting_Defaults['urlcolorscan']) == 1 ? 'true' : 'false') . ';';
		...
		function TS_VCSC_Extensions_Admin_Files($hook_suffix) {
			...
			if ($TS_VCSC_IsEditPagePost && $TS_VCSC_IsEditCustomPost == false) {				
				...
				if ($this->TS_VCSC_VCFrontEditMode == "false") {
					...
					if ($this->TS_VCSC_VisualComposer_Shortcodes == "true") {
				...
				wp_enqueue_style('ts-visual-composer-extend-composer');
				wp_enqueue_script('ts-visual-composer-extend-composer');
			...
			if (($ts_vcsc_main_page == $hook_suffix) || ($ts_vcsc_settings_page == $hook_suffix) || ($ts_vcsc_upload_page == $hook_suffix) || ($ts_vcsc_preview_page == $hook_suffix) || ($ts_vcsc_customCSS_page == $hook_suffix) || ($ts_vcsc_customJS_page == $hook_suffix) || ($ts_vcsc_system_page == $hook_suffix) || ($ts_vcsc_transfer_page == $hook_suffix) || ($ts_vcsc_license_page == $hook_suffix) || ($ts_vcsc_about_page == $hook_suffix) || ($ts_vcsc_google_fonts == $hook_suffix) || ($ts_vcsc_enlighterjs_page == $hook_suffix) || ($ts_vcsc_downtime_page == $hook_suffix) || ($ts_vcsc_sidebars_page == $hook_suffix) || ($ts_vcsc_statistics_page == $hook_suffix)) {
		...
		function TS_VCSC_DetermineLoadingStatus() {
			...
			if (($this->TS_VCSC_VCStandardEditMode == "false") && ($TS_VCSC_Extension_Request == "false") && (is_admin()) && ($TS_VCSC_Extension_RoleManager == "false") && (defined('WPB_VC_VERSION'))) {
		...
		function TS_VCSC_RegisterAllShortcodes() {
			if ($this->TS_VCSC_PluginSupport == "true") {
				...
				if (($this->TS_VCSC_Visual_Composer_Elements['TS Icon Info Box']['active'] == "true") && ($this->TS_VCSC_Visual_Composer_Elements['TS Creative Link']['active'] == "false")) {
					if (($this->TS_VCSC_VCFrontEditMode == "true") || (is_admin() == false) || ($this->TS_VCSC_PluginAJAX == "true") || ($this->TS_VCSC_PluginAlways == "true")) {
